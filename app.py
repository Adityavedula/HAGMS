...
def dashboard():
    ...
    today_logs = NutritionLog.query.filter_by(user_id=current_user.id, date=today).all()
    ...
    today_workout = WorkoutPlan.query.filter(
        WorkoutPlan.created_by == current_user.id, WorkoutPlan.date == today
    ).first()
    ...
    # Get recent workouts
    recent_workouts = (
        WorkoutPlan.query.filter_by(created_by=current_user.id)
    ...
    if latest_progress:
        ...
        old_progress = (
            Progress.query.filter(
                Progress.user_id == current_user.id, Progress.date <= month_ago
            )
            .order_by(Progress.date.desc())
            .first()
        )
...
def progress_logs():
    ...
    stats = {
        "total_logs": len(logs),
        "current_weight": logs[0].weight if logs else None,
        "current_bf": logs[0].body_fat_percentage if logs else None,
        "weight_change": None,
        "bf_change": None,
    }

    if len(logs) >= 2:
        stats["weight_change"] = logs[0].weight - logs[-1].weight
        if logs[0].body_fat_percentage and logs[-1].body_fat_percentage:
            stats["bf_change"] = (
                logs[0].body_fat_percentage - logs[-1].body_fat_percentage
            )

    return render_template("progress_logs.html", logs=logs, stats=stats)
...
def add_progress_log():
    if request.method == "POST":
        try:
            ...
            if not date_str or not weight:
                ...
                return redirect(url_for("add_progress_log"))
            ...
            flash(f"Error: Invalid input format. {str(e)}", "danger")
            return redirect(url_for("add_progress_log"))
    ...
    # For GET request
    # Get latest progress for pre-filling or showing current stats
    latest_progress = (
        Progress.query.filter_by(user_id=current_user.id)
        .order_by(Progress.date.desc())
        .first()
    ...
    return render_template(
        "add_progress_log.html",
        today=datetime.now().date(),
        latest_progress=latest_progress,
    )


@app.route("/edit_progress_log/<int:id>", methods=["GET", "POST"])
@login_required
def edit_progress_log(id):
    log = Progress.query.get_or_404(id)

    # Make sure the current user owns this log
    ...
    if request.method == "POST":
        try:
            ...
            log.weight = float(request.form["weight"])
            body_fat = request.form.get("body_fat_percentage")
            log.body_fat_percentage = float(body_fat) if body_fat else None
    ...
    return render_template("edit_progress_log.html", log=log)


@app.route("/delete_progress_log/<int:id>", methods=["POST"])
@login_required
...
@app.route("/progress_chart_data")
@login_required
def progress_chart_data():
    """API endpoint to get chart data for progress visualization"""
    logs = (
        Progress.query.filter_by(user_id=current_user.id)
    ...
    data = {
        ...
        "body_fat": [
            log.body_fat_percentage if log.body_fat_percentage else None for log in logs
        ],
    }
...
def nutrition_logs():
    ...
    avg_calories = db.session.query(
        func.avg(
            db.session.query(func.sum(NutritionLog.calories))
            .filter(
                NutritionLog.user_id == current_user.id,
                NutritionLog.date == date_subquery.c.date,
            )
    ...
    pagination = query.order_by(
        NutritionLog.date.desc(), NutritionLog.id.desc()
...
def export_nutrition_logs():
    ...
    if export_format == "csv":
        ...
        # Write header
        writer.writerow(
            ["Date", "Meal", "Calories", "Protein (g)", "Carbs (g)", "Fats (g)"]
        )
...
def nutrition_chart_data():
    ...
    daily_calories = (
        db.session.query(
            NutritionLog.date, func.sum(NutritionLog.calories).label("total_calories")
        )
        .filter(
            NutritionLog.user_id == current_user.id, NutritionLog.date >= start_date
        )
        .group_by(NutritionLog.date)
    ...
    meal_breakdown = (
        db.session.query(
            NutritionLog.meal, func.sum(NutritionLog.calories).label("total_calories")
        )
        .filter(
            NutritionLog.user_id == current_user.id, NutritionLog.date >= start_date
        )
        .group_by(NutritionLog.meal)
        .all()
    )
    ...
    meal_data = [
        {"meal": entry.meal, "calories": float(entry.total_calories)}
        for entry in meal_breakdown
    ]

    return jsonify(
        {"calories": calorie_data, "macros": macro_distribution, "meals": meal_data}
    )
...
def add_nutrition_log():
    if request.method == "POST":
        try:
            ...
            flash(
                f"Error: Invalid input format. Please check your values. {str(e)}",
                "danger",
            )
    ...
    daily_totals = {
        "calories": sum(log.calories for log in today_nutrition)
        if today_nutrition
        else 0,
        "protein": sum(log.protein for log in today_nutrition)
        if today_nutrition
        else 0,
    ...
    user_goals = {"calories": 2000, "protein": 150, "carbs": 250, "fats": 70}
            ...
            if exercise_names[i].strip():  # Only add if name is not empty
                ...
                if existing_exercise:
                    ...
                    existing_exercise.sets = (
                        int(exercise_sets[i]) if exercise_sets[i] else None
                    )
...
def edit_workout_plan(id):
    ...
    if request.method == "POST":
        ...
        plan.duration = (
            int(request.form["duration"]) if request.form["duration"] else None
        )
...
@app.route("/api/save_pose_session", methods=["POST"])
@login_required
